...
class KineticsReader(object):
    ...
    def __init__(self, mode, cfg, file_list):
        self.cfg = cfg
        self.mode = mode
        self.format = 'pkl'
        self.num_classes = cfg.num_class
        self.seg_num = cfg.num_segments
        self.seglen = 1

        self.short_size = cfg.short_size
        self.target_size = cfg.target_size
        self.num_reader_threads = cfg.workers
        self.buf_size = 1024

        self.img_mean = np.array(cfg.input_mean).reshape(
            [3, 1, 1]).astype(np.float32)
        self.img_std = np.array(cfg.input_std).reshape(
            [3, 1, 1]).astype(np.float32)
